...
def run(source_dir: Path | None = None) -> None:
    ...
    def stage(name, operation):
        ...
        try:
            ...
            stages.append({"stage_name": name, "status": "FAILED", "started_at": stage_started,
                           "ended_at": datetime.now(timezone.utc),
                           "duration_seconds": round(perf_counter() - tick, 3),
                           "records_processed": 0, "error_message": str(error)})
            raise
        stages.append({"stage_name": name, "status": "SUCCESS", "started_at": stage_started,
                       "ended_at": datetime.now(timezone.utc),
                       "duration_seconds": round(perf_counter() - tick, 3),
                       "records_processed": 0, "error_message": None})
        return result
    ...
    extracted_count = sum(
        len(source.frame) for source in sources if source.source_name != "PRODUCT_MASTER"
    )
    engine = get_engine()
    with engine.begin() as connection:
        run_id = connection.execute(text("""
            ...
            RETURNING run_id
        """), {"pipeline_name": settings.pipeline_name, "extracted_records": extracted_count}).scalar_one()
    ...
    try:
        ...
        incremental_results = stage("incremental_filter", lambda: filter_incremental_results(quality_results, existing_keys))
        ...
        valid_count = sum(result.summary["valid_records"] for result in quality_results.values() if result.summary["source"] != "product")
        duplicate_count = sum(result.summary["duplicate_records"] for result in quality_results.values() if result.summary["source"] != "product")
        invalid_count = sum(result.summary["invalid_records"] for result in quality_results.values() if result.summary["source"] != "product")
        with engine.begin() as connection:
            ...
            dimension_counts = stage("dimension_load", lambda: load_dimensions(connection, quality_results["product"].clean, staging_rows))
            ...
            connection.execute(text("""
                ...
                WHERE run_id = :run_id
            """), {"run_id": run_id, "valid_records": valid_count,
                    "duplicate_records": duplicate_count, "invalid_records": invalid_count,
                    "loaded_records": fact_inserted_count,
                    "validated_records": valid_count, "rejected_records": invalid_count,
                    "incremental_records": incremental_count, "staged_records": staged_count,
        ...
        LOGGER.info(
            "Pipeline succeeded: run_id=%s extracted_records=%s valid_records=%s "
            "duplicate_records=%s invalid_records=%s loaded_records=%s",
            run_id, extracted_count, valid_count, duplicate_count, invalid_count, fact_inserted_count,
        )
    except Exception as error:
        with engine.begin() as connection:
            connection.execute(text("""
                ...
                WHERE run_id = :run_id
            """), {"run_id": run_id, "duration_seconds": round(perf_counter() - started, 3),
