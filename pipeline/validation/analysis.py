...
CHECKS = {
    ...
    "invalid": {
        ...
        "NON_POSITIVE",
        "INVALID_INTEGER",
        "INVALID_MONEY_PRECISION_OR_RANGE",
        "AMOUNT_MISMATCH",
        "INVALID_STATUS",
        "PRICE_DIFFERS_FROM_MASTER",
        "INVALID_EMAIL",
        "UNKNOWN_PAYMENT",
    },
    "date": {"INVALID_DATE"},
...
def quality_issues(results, check=None):
    ...
    return issues.rename(
        columns={
            "source": "sumber",
            "source_row_number": "baris",
            "severity": "tingkat",
            "field": "kolom",
            "rule": "masalah",
            "raw_value": "nilai_asli",
        }
...
def missing_values(results):
    ...
    missing = pd.concat(parts, ignore_index=True).rename(columns={"column": "kolom", "missing_count": "jumlah_kosong"})
...
def summary(results):
    return pd.DataFrame([r.summary for r in results.values()]).rename(
        columns={
            "source": "sumber",
            "extracted_records": "awal",
            "valid_records": "bersih",
            "duplicate_records": "duplikat",
            "invalid_records": "ditolak",
        }
...
def export_analysis(results, output_dir=None):
    ...
    if set(results) == {"product", *SPECS}:
        ...
        for source, result in results.items():
            ...
            issues = issues.merge(
                payloads[["source_row_number", "raw_payload"]],
