...
def frame(rows, columns):
    ...
    for col in result:
        if col not in {'quantity', 'line_number', 'source_row_number', 'order_date', 'price',
                       'unit_price', 'gross_amount', 'source_total_amount'}:
...
def analyze(path, source, products=None):
    ...
    profile = pd.DataFrame([{'column': col, 'raw_dtype': str(data[col].dtype),
        'missing_count': sum(clean_text(v) is None for v in data[col]),
        'distinct_nonmissing': len({clean_text(v) for v in data[col]} - {None})} for col in data])
    lookup = {}
    if source != 'product':
    ...
    for offset, raw in enumerate(data.to_dict('records'), 1):
        ...
        def issue(code, field, severity='ERROR'):
            issues.append({**meta, 'severity': severity, 'rule': code, 'field': field,
        ...
        if source == 'product':
            ...
            for col in ['customer_name', 'customer_email', 'customer_city',
                        'store_name', 'store_city', 'payment_method']:
    ...
    for key, group in groups.items():
        signatures = {json.dumps({k: v for k, v in item['row'].items() if k not in META},
                                 sort_keys=True, default=str) for item in group}
        if len(signatures) > 1:
            conflicts.add(key)
    ...
    for candidate in candidates:
        ...
        if key in conflicts:
            ...
            issues.append({**meta, 'severity': 'ERROR', 'rule': 'CONFLICTING_DUPLICATE',
                           'field': 'business_key', 'raw_value': key})
        record = {**meta, 'business_key': key, 'reasons': '|'.join(errors),
        ...
        if errors:
            ...
            issues.append({**meta, 'severity': 'INFO', 'rule': 'DUPLICATE_BUSINESS_KEY',
                           'field': 'business_key', 'raw_value': key})
        else:
            seen.add(key)
            clean.append(row)
    ...
    summary = dict(source=source, source_sha256=checksum, extracted_records=len(data),
                   valid_records=len(clean), duplicate_records=len(duplicates), invalid_records=len(rejected),
                   warning_count=sum(i['severity'] == 'WARNING' for i in issues))
    assert len(data) == len(clean) + len(duplicates) + len(rejected)
    return QualityResult(frame(clean, columns), pd.DataFrame(rejected, columns=evidence_cols),
...
def run_quality(source_dir=None, output_dir=None):
    ...
    if output_dir is not None:
        ...
        manifest = {'policy_version': '1.0', 'source_files': [r.summary for r in results.values()],
                    'money': 'IDR Decimal, two decimal places', 'date': 'YYYY-MM-DD, calendar date without timezone',
