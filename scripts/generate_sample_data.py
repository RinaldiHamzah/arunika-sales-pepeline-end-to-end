...
CUSTOMERS = [
    ("C001", "Alya Putri", "Jakarta"),
    ("C002", "Bima Pratama", "Bandung"),
    ("C003", "Citra Lestari", "Surabaya"),
    ("C004", "Dimas Saputra", "Yogyakarta"),
    ("C005", "Eka Wulandari", "Semarang"),
]
...
def records(offset: int) -> list[dict]:
    ...
    for i in range(50):
        ...
        result.append(
            {
                "id": offset + i,
                "date": date(2026, 1, 1) + timedelta(days=RNG.randrange(240)),
                "sku": sku,
                ...
                "customer": customer,
                "city": city,
                "payment": RNG.choice(["Transfer Bank", "E-Wallet", "Credit Card", "COD"]),
                "status": RNG.choice(["completed", "completed", "completed", "cancelled", "returned"]),
                "customer_email": f"{customer.lower().replace(' ', '.')}@example.test",
            }
        )

    # Controlled quality defects, one of each type per source.
    result[3]["qty"] = 0
    result[7]["price"] = -50000
    result[11]["customer"] = ""
    result[15]["city"] = ""
    result[19]["payment"] = ""
    result[23]["status"] = "in_progress"
    result[27]["name"] = "Brigthning Serumm 30ml"
    result[31]["date"] = "not-a-date"
...
def main() -> None:
    ...
    write(
        ...
        lambda r: {
            ...
            "transaction_date": fmt(r["date"], "%d/%m/%Y"),
            "item_name": r["name"],
            "quantity": r["qty"],
            "price": r["price"],
            "buyer_name": r["customer"],
            "city": r["city"],
            "payment": r["payment"],
            "status": r["status"],
        },
    ...
    write(
        ...
        lambda r: {
            ...
            "created_at": fmt(r["date"], "%Y-%m-%dT10:30:00"),
            "product_identifier": r["sku"],
            "quantity": r["qty"],
            "unit_price": r["price"],
            "total_amount": r["qty"] * r["price"],
    ...
    write(
        ...
        lambda r: {
            ...
            "sold_at": fmt(r["date"], "%d-%b-%Y"),
            "item_description": r["name"],
            "units": r["qty"],
            "item_price": r["price"],
            "store_name": "Arunika Beauty Store",
            "store_city": r["city"],
    ...
    write(
        ...
        [dict(zip(["sku", "product_name", "brand", "category", "price"], product)) for product in PRODUCTS],
        lambda r: r,
    )
