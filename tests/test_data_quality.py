...
from pipeline.validation.data_quality import analyze, read_source, run_quality, ROOT
...
@pytest.fixture
def master(tmp_path):
    p = save(tmp_path / 'product.csv', [dict(sku='001', product_name='Brand SPF 50 PA++++ 30ml',
             brand='Brand', category='Skincare', price='100.10')])
    return analyze(p, 'product').clean


def web(order='00001', **changes):
    return dict(invoice_no=order, created_at='Apr 03, 2026', product_identifier='001',
                quantity='2', unit_price='100.10', total_amount='200.20',
                customer_email='buyer@example.test', status='completed', **changes)
...
def test_internal_clean_data_excludes_source_only_customer_id(tmp_path, master):
    row = {
        'order_id': 'SHP-1', 'order_date': '03/04/2026', 'product_name': '001',
        'qty': '1', 'unit_price': '100.10', 'customer_id': 'C001',
        'customer_name': 'Buyer', 'customer_city': 'Jakarta',
...
def test_invalid_required_and_amount_are_not_imputed(tmp_path, master):
    rows = [dict(web('1'), quantity=''), dict(web('2'), total_amount='200.21'),
            dict(web('3'), product_identifier='Unknown Product'), dict(web('4'), status='pending')]
    result = analyze(save(tmp_path / 'web.csv', rows), 'website', master)
